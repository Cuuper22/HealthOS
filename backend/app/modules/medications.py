...
class MedicationsModule(BaseModule):
    ...
    def description(self) -> str:
        return "Current and past medications with dosing notes."
    ...
    def has_data(self, user_id: str) -> bool:
        with SessionLocal() as session:
            count = session.execute(
                select(func.count()).select_from(Medication).where(Medication.user_id == user_id)
            ).scalar_one()
            return count > 0
    ...
    def get_last_update(self, user_id: str) -> datetime | None:
        with SessionLocal() as session:
            record = session.execute(
                select(Medication).where(Medication.user_id == user_id).order_by(Medication.updated_at.desc())
            ).scalars().first()
